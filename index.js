...
async function sendNotificationAndUpdateDocuments(alertTableId, userId, eventId, alertDoc) {
  ...
  return db.runTransaction(async (transaction) => {
    const alertData = await transaction.get(alertRef);

    if (!alertData.exists || alertData.data().isProcessing) {
      return; // Document doesn't exist or already being processed, exit
    }

    transaction.update(alertRef, { isProcessing: true }); // Set flag within transaction
    ...
    try {
      ...
      transaction.update(alertRef, { isProcessing: false }); // Remove flag after processing
    }
  });
}
